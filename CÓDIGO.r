...
musicas <- subset(musicas, select = -c(released_month, released_day, acousticness_.,instrumentalness_.,liveness_.,speechiness_., cover_url,valence_.,in_shazam_charts,in_deezer_charts,in_deezer_playlists,in_apple_charts,in_apple_playlists))
names(musicas)
...
musicas$streams <- as.numeric(gsub(",", "", musicas$streams))
...
ggplot(ano_mais_hits_2010, aes(x = released_year, y = total_streams_2010)) +
  geom_bar(stat = "identity", fill = "darkgoldenrod3", col = "black") +
  scale_y_continuous(labels = comma) +
...
ggplot(musicas, aes(x = bpm, y = streams)) +
  geom_point(color = "darkorchid", alpha = 0.5) +
  scale_y_continuous(labels = comma) +
...
ggplot(sza, aes(x = reorder(track_name, streams), y = streams)) +
  geom_bar(stat = "identity", fill =  "violet"   , col = "black") +
  scale_y_continuous(labels = comma) +
